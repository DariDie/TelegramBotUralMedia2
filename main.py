...
def start(message):
    ...
    bot.send_message(message.from_user.id, 'Привет!🔥 Судя по всему, ты собираешься в путешествие, но ещё не определился с местом поездки. Я бот, который поможет тебе с выбором - предлагаю тебе путешествие в Ханты-Мансийский АО, а именно в город Нижневартовск, который превздоёт все твои ожидания ⬆', reply_markup=user_markup)
