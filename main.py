...
def text(message):
    if message.text == 'Прогулки':
        ...
        bot.send_message(message.from_user.id, ':one: Набережная - самое популярное место для отдыха и прогулок :+1: Это своеобразная визитная карточка города, поскольку именно река Обь является его историческим началом :city_sunrise:\n:two: Рябиновый бульвар - пешеходная улица, выполненная с различными архитектурными конструкциями и решением сохранения зелёного фонда :seedling: Кроме того, на бульваре можно увидеть представителей местной фауны - красавцев-снегирей 🥰\n:three: Парк победы - важное место для города и его истории. Внутри же него, помимо мемориала и скульптур, можно найти скромный, но не менее интересный парк аттракционов :ferris_wheel:\n:four: Граффити, которые ты сможешь встретить на своем маршруте. Говорят, на их фоне получаются самые удачные фото :fire:')
